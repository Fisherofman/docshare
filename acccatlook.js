...
var gbacklook,loc,gicallup=false;
function log(ss) {  fs.log(ss);  }
//************************************** main routine *********************************************************************
function accmenu(sarg,sdat,sback,sbuf) {

var gwidth,gheight,ghmid,gwr,ghr,glay={},gx,gy,gba=false,comm=[],lbuf=[];
var lmode,bmess=false,llogin,llock,sip,lfile='cust',lindex='desc';
function keyconv(ss) {     return(fs.keyconvert(sip,ss,gkeymy)); }    

function initgen(sdat,ss) { 
  if(ss!=undefined)  gbacklook=ss;  
  sip=gnodeselect;  
...
accmenu.prototype.but3click = function (n) { 
            if(n==1) {  fs.run('root','maathelp','acccatlook,Categories Lookup');  }
       else if(n==2) { fs.run('root',gbacklook,comm,'',sbuf);    }
       else if(n==3) {  fs.run('root','acccatadd',comm,'acccat');}
...
accmenu.prototype.keysup = function (e) { 
     if(gicallup==false) {  gicallup=true;    setTimeout(function() {  searchreal(c.val('idsearch')); },500);   } 
}

accmenu.prototype.butedit = function (ss) { //log(ss+'...Name');
  var hd,sb,s1;
     fs.run('root',gbacklook,comm,'',sbuf,ss+'.cat'); 
}    
//**************************************** display *****************************************************
function searchreal(ss) {
 var sa='',s2;
         ...
         sa=sa+c.tr(
               ...
               c.table( c.at(c.cellspacing(0),c.cellpadding(0)),
                   c.tr( c.td( c.at(c.att(c.height(20*gy))) )),
                      c.tr( c.td(  c.link( c.at(c.href('#'),  c.onclick('loc.butedit("'+s2+'")')),
                        c.table( c.at(c.cellspacing(0),c.cellpadding(0)),
...
function searchret(ss) { 
     ...
     if(fs.dfind(hdc,lindex,'item','item','',100)==true) { //sb='';
       fs.dfirst(hdc);
       for(;;) {
   ...
   ss=c.svg(  c.at(c.id('idmain'),c.nwidth(gwidth),c.cheight(ksep)), //,c.viewbox(0,0,gwr,210),c.viewbox(0,0,gwr,210),c.viewbox('0 0 100 100') c.att(c.background_image(sgrp))
     ...
     c.defs(c.stylesheet(c.font_face(
        c.st(" font-family: 'myfont';  src: url(data:font/ttf;base64,"+gmfont+") format('truetype');  font-weight: normal; font-style: normal;")    
           ...
           //*******************  heading *****************************************************************
           c.text(c.at(c.xy(15,60),c.onclick('loc.but3click(1)'),c.att(c.fill(fs.theme(1)),c.font_weight(fs.theme(3)),c.fill_opacity(0.8),c.sfont_size(55),c.cursor('pointer'),c.font_family('myfont'))),'Catogories Lookup'),
...
//           c.image(c.xywh(kofs+kadd*1,ktop,kw,kh),c.xlink(fs.picsvg('xsetting')),c.onclick('loc.but3click(6)'),c.att(c.cursor('pointer'))           ),
        ...
        //   c.image(c.xywh(kofs+kadd*3,ktop,kw,kh),c.xlink(fs.picsvg('xdesc')),c.onclick('loc.but3click(4)'),c.att(c.cursor('pointer'))           ),
