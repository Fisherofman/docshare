...
var gbackshopperadd,loc,gicallup=false;
//************************************** main routine *********************************************************************
function accmenu(sarg,sdat,sback) {
var gwidth,gheight,ghmid,gwr,ghr,glay={},gx,gy,gba=false,sip,comm,banew=false,ksep=210;
var lmode,bmess=false,llogin,llock,lupdate='',lfile='cust',lfields='shop,cat,desc,memo';
...
function initgen(sdat,ss) { 
 if(ss!=undefined)  gbackshopperadd=ss; sip=gnodeselect;  
...
accmenu.prototype.but3click = function (n) { 
            if(n==1) {  fs.run('root','maathelp','accshopper,Shopper Add');  }
       else if(n==2) {   fs.run('root',gbackshopperadd,comm);  }
       ...
       sa+=c.tr( 
          ...
          c.textarea(
               ...
               c.att(c.border('1px solid #b0b0b0'),
                  c.sfont_size(32),c.font_family('myfont'),c.st('outline-color:#404040; '),c.white_space('nowrap'),c.swh(580-iofs-ioff,(ghr-ksep)-(120+24))
               )
              ),sublay(sb)
           )
       ));
   ...
   ss=c.svg(  c.at(c.id('idmain'),c.nwidth(gwidth),c.cheight(ksep)), //,c.viewbox(0,0,gwr,210),c.viewbox(0,0,gwr,210),c.viewbox('0 0 100 100') c.att(c.background_image(sgrp))
     ...
     c.defs(c.stylesheet(c.font_face(
        c.st(" font-family: 'myfont';  src: url(data:font/ttf;base64,"+gmfont+") format('truetype');  font-weight: normal; font-style: normal;")    
           ...
           //*******************  heading *****************************************************************
           c.text(c.at(c.xy(15,60),c.onclick('loc.but3click(1)'),c.att(c.fill(fs.theme(1)),c.font_weight(fs.theme(3)),c.fill_opacity(0.8),c.sfont_size(55),c.cursor('pointer'),c.font_family('myfont'))),'Add Shopper'),
...
//          c.polygon(c.points(20,140,40,160,40,120),c.att(c.fill('#FFFFFF')) ),
          ...
          c.input(c.at(
                  c.ntype('text'),c.hint('Shopper Name'),c.id('idsearch'),c.styl('autocomplete','off'),c.spellcheck(false),c.value(sf),c.onkeyup('loc.keysup(event)'),   
...
//********************************* main run **************************************
      ...
      searchret(sdat); if(sdat=='') {  c.val('idsearch',''); } else {  c.nstyle('iddel','display','block'); }
