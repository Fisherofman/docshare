...
// declare var gmicon,gmback,gmlock,gmfont,gmwidth,gmwidthreal,gloccopy,gchartline,gchartshop;
...
var gbackshop,loc,gicallup=false; // cust stock (names)
...
function initgen(sdat,ss) { 
  var sa;
  if(ss!=undefined) { gbackshop=ss;  gchartline='spec';  }
  sip=gnodeselect;  
  ltosend=sdat; 
  laccount=fs.xitemget(ltosend,2,'|')
...
function butshopperselect() {
    ...
    //log('\n'+s2+'xxxxx');    s2=bu.join('\n');
    if(ba==true) { //log('........'+bu.length+':'+gchartshop[0]);
        hdc=fs.dopen(sip,'shop'+lname+'shop');  if(hdc==0) return('');
...
accmenu.prototype.but3click = function (n) { 
            if(n==1) {  fs.run('root','maathelp','accshop,Shop',ltosend);  }
       else if(n==2) {  butshopperselect(); fs.run('root',gbackshop,ltosend);  }
   ...
   ss=c.svg(  c.at(c.id('idmain'),c.nwidth(gwidth),c.cheight(ksep)), //,c.viewbox(0,0,gwr,210),c.viewbox(0,0,gwr,210),c.viewbox('0 0 100 100') c.att(c.background_image(sgrp))
     ...
     c.defs(c.stylesheet(c.font_face(
        c.st(" font-family: 'myfont';  src: url(data:font/ttf;base64,"+gmfont+") format('truetype');  font-weight: normal; font-style: normal;")    
           ...
           //*******************  heading *****************************************************************
           c.text(c.at(c.xy(15,60),c.onclick('loc.but3click(1)'),c.att(c.fill(fs.theme(1)),c.font_weight(fs.theme(3)),c.fill_opacity(0.8),c.sfont_size(55),c.cursor('pointer'),c.font_family('myfont'))),'Shop'),
        ...
        //   c.image(c.xywh(kofs+kadd*0,ktop,kw,kh),c.xlink(fs.picsvg('xsetting')),c.onclick('but3click(7)'),c.att(c.cursor('pointer'))           ),
           ...
           c.image(c.xywh(kofs+kadd*0,ktop,kw,kh),c.xlink(fs.picsvgi('xchart')),c.att(c.cursor('pointer'))           ),
           c.rect(c.id('idkey5'), c.xywh(kofs+kadd*0,ktop-5,kw,kh+10),c.onclick('loc.but3click(6)'),c.att(c.fill('transparent'),c.display('block'))), //back selectiom

           c.rect(c.id('idkey4'), c.xywh(kofs+kadd*1,ktop-5,kw,kh+10),c.att(c.fill(fs.theme(6)),c.display('none'))), //back selectiom
           c.image(c.xywh(kofs+kadd*1,ktop,kw,kh),c.xlink(fs.picsvgi('xusersmall')),c.onclick('loc.but3click(8)'),c.att(c.cursor('pointer'))           ),
