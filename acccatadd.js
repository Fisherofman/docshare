...
var gbackcatadd,loc,gicallup=false;
//************************************** main routine *********************************************************************
function accmenu(sarg,sdat,sback) {
var gwidth,gheight,ghmid,gwr,ghr,glay={},gx,gy,gba=false,sip,comm,banew=false,ksep=210;
var lmode,bmess=false,llogin,llock,lupdate='',lfile='cust',lfields='cat,desc,memo',lback=false,lname,lpass; 
...
function initgen(sdat,ss) { 
   if(ss=='acccust') { } else {      if(ss!=undefined)  gbackcatadd=ss;     }
   sip=gnodeselect;   lfile=keyconv('stockcat');   lname=fs.dkeyhalf(sip,gnodename);  lpass='shop'+lname+'cat'
   if(typeof(sdat)=='object') {  comm=sdat;  sdat=''; lback=true; }
   return(sdat);
}
...
function custget() {
           comm=[];  comm.push(c.val('idsearch')); comm.push(lupdate);
...
function custput() { 
 ...
 if(comm.length>0)  { } 
    c.val('idsearch',comm[j]); j++;  lupdate=comm[j]; ln=fs.itemcount(lfields);
    for(var i=2;i<=ln;i++) { j++;          
             c.val('id'+fs.itemget(lfields,i),comm[j]); 
     }
     j++; ss=comm[j]; 
     if(ss=='stock') { 
            j++; c.val('idmemo',c.val('idmemo')+'\n'+comm[j]+' '+comm[j+1]);
    } 
    comm=[];
}    
...
accmenu.prototype.but3click = function (n) { 
            if(n==1) {  fs.run('root','maathelp','acccatadd,Category Add');  }
       else if(n==2) {   fs.run('root',gbackcatadd,comm);  }
       else if(n==3) {  fs.run('root','acccat',comm,'accmenu');}
       else if(n==20) {  setupkeyadd();  }
       else if(n==80) {   custget();   fs.run('root','accstock',comm,'acccatadd');         }
...
//********************************************************************
    ...
    hd=fs.dopen(sip,lfile,lpass);     if(hd==0) { return;    } 
     if(lupdate=='') {
       ...
       sa+=c.tr( 
          ...
          c.textarea(
               ...
               c.att(c.border('1px solid #b0b0b0'),
                  c.sfont_size(32),c.font_family('myfont'),c.st('outline-color:#404040; '),c.white_space('nowrap'),c.swh(580-iofs-ioff,(ghr-ksep)-(60+12))
               )
              ),sublay(sb)
           )
       ));
...
function searchret(ss) { lupdate=ss; //log(ss);
    ...
    if(ss=='') { sa=fieldlay(0,sfld); } else {
     ...
     if(hd==0) { sa=fieldlay(0,sfld); } else {
      if(fs.dfindid(hd,ss,1)==true) {  s1=fs.dnno(hd,1).trim(); sa=fieldlay(hd,sfld);     }
      fs.dclose(hd);
     }
    }
...
var x,y,sa,sb,k,j=0,jj=0;
 ...
 function initsecond(sf) { 
  var ss,s1,kofs=200,ktop=20,kw=50,kh=50,kadd=60,sx; 
   ...
   ss=c.svg(  c.at(c.id('idmain'),c.nwidth(gwidth),c.cheight(ksep)), //,c.viewbox(0,0,gwr,210),c.viewbox(0,0,gwr,210),c.viewbox('0 0 100 100') c.att(c.background_image(sgrp))
     ...
     c.defs(c.stylesheet(c.font_face(
        c.st(" font-family: 'myfont';  src: url(data:font/ttf;base64,"+gmfont+") format('truetype');  font-weight: normal; font-style: normal;")    
           ...
           //*******************  heading *****************************************************************
           c.text(c.at(c.xy(15,60),c.onclick('loc.but3click(1)'),c.att(c.fill(fs.theme(1)),c.font_weight(fs.theme(3)),c.fill_opacity(0.8),c.sfont_size(55),c.cursor('pointer'),c.font_family('myfont'))),'Add Category'),
        ...
        //   c.image(c.xywh(kofs+kadd*3,ktop,kw,kh),c.xlink(fs.picsvg('xsetting')),c.onclick('but3click(4)'),c.att(c.cursor('pointer'))           ),
          c.image(c.xywh(kofs+kadd*3,ktop,kw,kh),c.xlink(fs.picsvgi('xdesc')),c.onclick('loc.but3click(80)'),c.att(c.display('block'),c.cursor('pointer'))           ),
...
//********************************* main run **************************************
      ...
      searchret(sdat); 
      if(lback) { 
        custput();   c.nstyle('iddel','display','block'); 
      } else {
       if(sdat=='') {  c.val('idsearch',custno(1)); } else {  c.nstyle('iddel','display','block'); }
      }  
