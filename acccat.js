...
// declare var gservpubkey,gpubkeymy,gnodename,gkeymy,gpubkeymyhash,gnodeselect,gnodemain,gnodemaster,gcustlist,gkeypmy,gpic,gpics,gscripts,gnodeloc,glocal,gappback;
var gbackcat,loc,gicallup=false;
function log(ss) {  fs.log(ss);  }
//************************************** main routine *********************************************************************
function accmenu(sarg,sdat,sback) {

var gwidth,gheight,ghmid,gwr,ghr,glay={},gx,gy,gba=false,comm=[];
var lmode,bmess=false,llogin,llock,sip,lfile='cust',lindex='desc',lname,lpass;
function keyconv(ss) {     return(fs.keyconvert(sip,ss,gkeymy)); }    

function initgen(sdat,ss) { 
  if(ss!=undefined)  gbackcat=ss;  
  sip=gnodeselect;  
  if(typeof(sdat)=='object') {  comm=sdat;  }
  lfile=keyconv('stockcat'); 
  lname=fs.dkeyhalf(sip,gnodename); 
  lpass='shop'+lname+'cat'
}

accmenu.prototype.but3click = function (n) { 
            if(n==1) {  fs.run('root','maathelp','acccat,Categories');  }
       else if(n==2) {  fs.run('root',gbackcat);  }
       else if(n==3) {  fs.run('root','acccatadd','','acccat');}
       else if(n==4) { c.nstyle('idkey1','display','block'); c.nstyle('idkey2','display','none');   lindex='desc'; searchret('');   }
       else if(n==5) { c.nstyle('idkey1','display','none'); c.nstyle('idkey2','display','block');  lindex='cat'; searchret('');   }
  ...
  var hd,sb,s1;
    ...
    //   }
     if(comm.length>0) {
        ...
        if(fs.dfindid(hd,ss,1)==true) { comm.push('cat'); s1=fs.dn(hd,'cat').trim(); comm.push(s1); s1=fs.dn(hd,'desc').trim(); comm.push(s1);    }
       fs.dclose(hd);
       fs.run('root',gbackcat,comm,'acccust');
     } else {     fs.run('root','acccatadd',ss,'acccat'); }
}    
//**************************************** display *****************************************************
function searchret(ss) { 
     var sa='',sc,sb='',hdc,kk=0,sline,sno;
     hdc=fs.dopen(sip,lfile,lpass); if(hdc==0) return;
...
//          sb=kk+'.'+fs.dn(hdc,'name')+':'+fs.dn(hdc,'memo'); kk++; 
          sb=fs.dn(hdc,'desc').trim(); sline=fs.dn(hdc,'cat').trim()+'   '+fs.dn(hdc,'desc').trim(); sno=fs.dn(hdc,'cat').trim();
          //************************************************************************
             sa=sa+c.tr(
               c.td(
   ...
   ss=c.svg(  c.at(c.id('idmain'),c.nwidth(gwidth),c.cheight(ksep)), //,c.viewbox(0,0,gwr,210),c.viewbox(0,0,gwr,210),c.viewbox('0 0 100 100') c.att(c.background_image(sgrp))
     ...
     c.defs(c.stylesheet(c.font_face(
        c.st(" font-family: 'myfont';  src: url(data:font/ttf;base64,"+gmfont+") format('truetype');  font-weight: normal; font-style: normal;")    
           ...
           //*******************  heading *****************************************************************
           c.text(c.at(c.xy(15,60),c.onclick('loc.but3click(1)'),c.att(c.fill(fs.theme(1)),c.font_weight(fs.theme(3)),c.fill_opacity(0.8),c.sfont_size(55),c.cursor('pointer'),c.font_family('myfont'))),'Catogories'),
...
//           c.image(c.xywh(kofs+kadd*1,ktop,kw,kh),c.xlink(fs.picsvg('xsetting')),c.onclick('loc.but3click(6)'),c.att(c.cursor('pointer'))           ),
           ...
           c.rect(c.id('idkey2'), c.xywh(kofs+kadd*2,ktop-5,kw,kh+10),c.att(c.fill(fs.theme(6)),c.display('none'))), //back selectiom
           c.image(c.xywh(kofs+kadd*2,ktop,kw,kh),c.xlink(fs.picsvgi('xsetting')),c.onclick('loc.but3click(5)'),c.att(c.cursor('pointer'))           ),
           
           c.rect(c.id('idkey1'), c.xywh(kofs+kadd*3,ktop-5,kw,kh+10),c.att(c.fill(fs.theme(6)),c.display('block'))), //back selectiom
           c.image(c.xywh(kofs+kadd*3,ktop,kw,kh),c.xlink(fs.picsvgi('xdesc')),c.onclick('loc.but3click(4)'),c.att(c.cursor('pointer'))           ),
...
//          c.polygon(c.points(20,140,40,160,40,120),c.att(c.fill('#FFFFFF')) ),
          ...
          c.input(c.at(
                  c.ntype('text'),c.hint('Category Search'),c.id('idsearch'),c.styl('autocomplete','off'),c.spellcheck(false),c.value(sf),c.onkeyup('loc.keysup(event)'),   
