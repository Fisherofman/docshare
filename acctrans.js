...
var gbacktrans,loc,gicallup=false; // cust stock (names)
...
function initgen(ss) { if(ss!=undefined)  gbacktrans=ss;  sip=gnodeselect;  
    lfile=keyconv('trans'); 
...
accmenu.prototype.but3click = function (n) { 
       ...
       else if(n==2) {  fs.run('root',gbacktrans);  }
...
function searchret(ss) { 
     var sa='',sc,sb='',hdc,kk=0,sline,sno,hdi,sno,ino,imany=100;
     hdi=fs.dopen(sip,keyconv('cust'));
     hdc=fs.dopen(sip,lfile); if(hdc==0) return;
     
     sno=fs.dataget(sip,lfile+'no','number',1); ino=fs.xis(sno);
     if(ino>(imany+1000)) { ino=ino-imany; ss=ino+''; }
     if(fs.dfind(hdc,lindex,ss,'','',100)==true) { //sb='';
       fs.dlast(hdc);
       for(;;) {
         if(fs.deof(hdc)==true) break;
         if(fs.dn(hdc,'del')=='X') { fs.dprev(hdc); continue; }
//          sb=kk+'.'+fs.dn(hdc,'name')+':'+fs.dn(hdc,'memo'); kk++; 
          sb=fs.dn(hdc,'ref').trim()+' '+custget(hdi,fs.dn(hdc,'acc')); 
          sline=fs.dn(hdc,'ref').trim()+'   '+fs.dn(hdc,'desc').trim()+'   '+fs.dn(hdc,'acc').trim()+'   '+fs.dn(hdc,'period').trim(); 
          sno=fs.dn(hdc,'ref').trim();
          //************************************************************************
             sa=sa+c.tr(
               c.td(
   ...
   ss=c.svg(  c.at(c.id('idmain'),c.nwidth(gwidth),c.cheight(ksep)), //,c.viewbox(0,0,gwr,210),c.viewbox(0,0,gwr,210),c.viewbox('0 0 100 100') c.att(c.background_image(sgrp))
     ...
     c.defs(c.stylesheet(c.font_face(
        c.st(" font-family: 'myfont';  src: url(data:font/ttf;base64,"+gmfont+") format('truetype');  font-weight: normal; font-style: normal;")    
           ...
           //*******************  heading *****************************************************************
           c.text(c.at(c.xy(15,60),c.onclick('loc.but3click(1)'),c.att(c.fill(fs.theme(1)),c.font_weight(fs.theme(3)),c.fill_opacity(0.8),c.sfont_size(55),c.cursor('pointer'),c.font_family('myfont'))),'Transactions'),
        ...
        //   c.image(c.xywh(kofs+kadd*0,ktop,kw,kh),c.xlink(fs.picsvg('xsetting')),c.onclick('but3click(7)'),c.att(c.cursor('pointer'))           ),
           c.rect(c.id('idkey3'), c.xywh(kofs+kadd*1,ktop-5,kw,kh+10),c.att(c.fill(fs.theme(6)),c.display('none'))), //back selectiom
           c.image(c.xywh(kofs+kadd*1,ktop,kw,kh),c.xlink(fs.picsvgi('xsetting')),c.onclick('loc.but3click(6)'),c.att(c.cursor('pointer'))           ),
           ...
           c.image(c.xywh(kofs+kadd*3,ktop,kw,kh),c.xlink(fs.picsvgi('xsetting')),c.onclick('loc.but3click(4)'),c.att(c.cursor('pointer'))           ),
...
//          c.polygon(c.points(20,140,40,160,40,120),c.att(c.fill('#FFFFFF')) ),
          ...
          c.input(c.at(
                  c.ntype('text'),c.hint('Transaction Search'),c.id('idsearch'),c.styl('autocomplete','off'),c.spellcheck(false),c.value(sf),c.onkeyup('loc.keysup(event)'),   
