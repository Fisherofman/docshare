...
accmenu.prototype.but3click = function (n) { 
            if(n==1) {  fs.run('root','maathelp','acccust,Customers');  }
       else if(n==2) {  fs.run('root',gbackcust);  }
       else if(n==3) {  fs.run('root','acccustadd',comm,'acccust');}
  ...
  var hd,sb,s1;
    ...
    //   }
     if(comm.length>0) {
       ...
       fs.dclose(hd);
       fs.run('root',gbackcust,comm,'acccust');
     } else {     fs.run('root','acccustadd',ss,'acccust'); }
}    
//**************************************** display *****************************************************
...
//          sb=kk+'.'+fs.dn(hdc,'name')+':'+fs.dn(hdc,'memo'); kk++; 
          sb=fs.dn(hdc,'name').trim(); sline=fs.dn(hdc,'acc').trim()+'   '+fs.dn(hdc,'name').trim()+'   '+fs.dn(hdc,'contact').trim(); sno=fs.dn(hdc,'acc').trim();
          //************************************************************************
             sa=sa+c.tr(
               c.td(
   ...
   ss=c.svg(  c.at(c.id('idmain'),c.nwidth(gwidth),c.cheight(ksep)), //,c.viewbox(0,0,gwr,210),c.viewbox(0,0,gwr,210),c.viewbox('0 0 100 100') c.att(c.background_image(sgrp))
     ...
     c.defs(c.stylesheet(c.font_face(
        c.st(" font-family: 'myfont';  src: url(data:font/ttf;base64,"+gmfont+") format('truetype');  font-weight: normal; font-style: normal;")    
           ...
           //*******************  heading *****************************************************************
           c.text(c.at(c.xy(15,60),c.onclick('loc.but3click(1)'),c.att(c.fill(fs.theme(1)),c.font_weight(fs.theme(3)),c.fill_opacity(0.8),c.sfont_size(55),c.cursor('pointer'),c.font_family('myfont'))),'Customers'),
        ...
        //   c.image(c.xywh(kofs+kadd*0,ktop,kw,kh),c.xlink(fs.picsvg('xsetting')),c.onclick('but3click(7)'),c.att(c.cursor('pointer'))           ),
           ...
           c.image(c.xywh(kofs+kadd*1,ktop,kw,kh),c.xlink(fs.picsvgi('xcat')),c.onclick('loc.but3click(6)'),c.att(c.cursor('pointer'))           ),
...
//          c.polygon(c.points(20,140,40,160,40,120),c.att(c.fill('#FFFFFF')) ),
          ...
          c.input(c.at(
                  c.ntype('text'),c.hint('Customer Search'),c.id('idsearch'),c.styl('autocomplete','off'),c.spellcheck(false),c.value(sf),c.onkeyup('loc.keysup(event)'),   
