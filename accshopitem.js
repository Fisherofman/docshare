...
// declare var gstart,gservpubkey,gpubkeymy,gkeymy,gpubkeymyhash,gnodeselect,gnodemain,gnodemaster,gcustlist,gkeypmy,gpic,gpics,gscripts,gnodeloc,glocal,gappback,gsess;
...
var lmode,bmess=false,llogin,llock,lupdate='',lfile='cust',lremark;
...
//  lhtml='shop'+laccount+'html';
 //sa=fs.dkeyname(sip,laccount); lremark='99|'+laccount+'|'+fs.xhexs(sa['pub'])+':'+lno+':'+lno+':'+lno+':';
 lname=fs.dkeyhalf(sip,laccount); // log(lremark);
 lremark='99|'+laccount+'|'+fs.xitemget(sinfo,3,'|')+':'+lno+lname+':'+lno+lname+':'+lno+lname+'|'; //log(lremark)
 lfile='shop'+lname; 
 lpass='shop'+lname; sa=fs.dkeyname(sip,lpass); lremark='99|'+laccount+'|'+fs.xhexs(sa['pub'])+':'+lno+lname+':'+lno+lname+':'+lno+lname+'|';
...
accmenu.prototype.but3click = function (n) {
    var sa;
           ...
           //sa='2|'+gcustlist[2].name+'|'+gcustlist[2].address+'|'+gcustlist[2].pubkey+',';
           gsess=undefined;
           fs.run('root','accshopremark',lremark,lno,sinfo); 
...
function fieldlay(hd) {
    var sa='',sb,sc='',s1,iofs=18,ioff=2,iper=10,irat=10,ifont=35,scola='#FF8040',scode,sprice,sdesc;
    if(fs.xis(fs.dn(hd,'onhand'))>0) { s1='In Stock'; } else { s1='Out of Stock'; }
    scode=fs.dn(hd,'code'); gchartshop.push(scode+' '+fs.dn(hd,'desc')); sdesc=fs.dn(hd,'desc'); sdesc=fs.strout(sdesc,'|'); sdesc=fs.strout(sdesc,',');
    sprice=fs.dn(hd,'price').trim(); lremark=fs.xitemrep(lremark,sdesc,2,'|'); 
    sa+=c.tr( c.td(
            c.table( c.at(c.cellspacing(0),c.cellpadding(7),c.att(c.sfont_size(ifont),c.font_family('Arial')) ),
              ...
              c.tr( c.td(  'Quantity:'), c.td(                 //qnt
               ...
               c.tr( c.td(), c.td ( 
                   ...
                   c.att(c.border_radius('5px'),c.border('1px solid #b0b0b0'),c.sfont_size(ifont),c.swh(200,70),c.color('#FFFFFF'),c.background_color('#FF6060')),
   ...
   ss=c.svg(  c.at(c.id('idmain'),c.nwidth(gwidth),c.cheight(ksep)), //,c.viewbox(0,0,gwr,210),c.viewbox(0,0,gwr,210),c.viewbox('0 0 100 100') c.att(c.background_image(sgrp))
     ...
     c.defs(c.stylesheet(c.font_face(
        c.st(" font-family: 'myfont';  src: url(data:font/ttf;base64,"+gmfont+") format('truetype');  font-weight: normal; font-style: normal;")    
           ...
           //*******************  heading *****************************************************************
           c.text(c.at(c.xy(80+15,60),c.onclick('loc.but3click(1)'),c.att(c.fill(fs.theme(1)),c.font_weight(fs.theme(3)),c.fill_opacity(0.8),c.sfont_size(55),c.cursor('pointer'),c.font_family('myfont'))),'Order'),
           c.image(c.xywh(80,7,15,15),c.xlink(fs.picsvg('xhelp')),c.att(c.cursor('pointer'))           ),
          //******************** incons *************************************************************************************

          // c.image(c.xywh(kofs+kadd*3,ktop,kw,kh),c.xlink(fs.picsvgi('xdesc')),c.att(c.cursor('pointer'))           ),
          // c.rect(c.id('idkey2'), c.xywh(kofs+kadd*3,ktop-5,kw,kh+10),c.onclick('loc.but3click(7)'),c.att(c.fill('transparent'),c.display('block'))), //back selectiom


           c.image(c.xywh(kofs+kadd*4,ktop,kw,kh),c.xlink(fs.picsvgi('xchart')),c.att(c.cursor('pointer'))           ),
